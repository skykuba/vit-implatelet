...
def unfreeze_last_blocks(self, n):
    # 1. Odblokuj head (klasyfikator)
    for param in self.head.parameters():
        param.requires_grad = True
    
    # 2. Odblokuj ostatnie n bloków
    ...
    # 3. Odblokuj końcową normalizację
    for param in self.norm.parameters():
        param.requires_grad = True
    print(f"Unfrozen last {n} blocks and head.")

# Przypisanie metody do Twojego modelu
base_model.unfreeze_last_blocks = types.MethodType(unfreeze_last_blocks, base_model)
...
# # Odblokuj tylko Twój nowy head
# for param in base_model.head.parameters():
#     param.requires_grad = True

# Teraz optimizer w Fazie 1 będzie widział tylko head
optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), Config.PHASE1_LR)
